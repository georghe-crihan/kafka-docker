# Below comes a patched version of SimpleClient to talk to a specific Broker,
# including replicas.
...
class DevopsSimpleClient(object):
    ...
    # socket timeout.
    def __init__(self, hosts, specific_broker=None, client_id=CLIENT_ID,
        ...
        self._conns = {}
        self._specific_broker = specific_broker # host:port
    ...
    def _send_broker_unaware_request(self, payloads, encoder_fn, decoder_fn):
        ...
        if not self._specific_broker:
          ...
          host, port, afi = get_ip_port_afi(self._specific_broker)
          hosts.add((host, port, afi))
        ...
        if not self._specific_broker:
          ...
          for nodeId, host, port in resp.brokers:
            if str(host) + ':' + str(port) == self._specific_broker:
              self.brokers = {nodeId: BrokerMetadata(nodeId, host, port, None)} 
              break
